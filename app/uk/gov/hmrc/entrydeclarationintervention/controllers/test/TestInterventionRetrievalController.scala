...
import uk.gov.hmrc.entrydeclarationintervention.services.InterventionRetrievalService
...
class TestInterventionRetrievalController @Inject()(cc: ControllerComponents, service: InterventionRetrievalService)(
  ...
  def getFullIntervention(eori: String, notificationId: String): Action[AnyContent] = Action.async { _ =>
    service.retrieveFullIntervention(eori, notificationId) map {
      case Some(intervention) => Ok(Json.toJson(intervention))
      case None                   => NotFound
    }
  }
}
